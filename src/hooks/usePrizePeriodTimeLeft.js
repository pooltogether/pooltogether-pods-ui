...
export const usePrizePeriodTimeLeft = (pool) => {
  if (
    ...
    const prizePeriodDurationInSeconds = BigNumber.from(pool.prize.prizePeriodSeconds).toNumber()
    const prizePeriodStartedAtInSeconds = BigNumber.from(pool.prize.prizePeriodStartedAt).toNumber()
